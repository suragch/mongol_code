...
class MongolWord {
  ...
  String convertToMenksoftCode() {
    ...
    StringBuffer renderedWord = StringBuffer();
    int charBelow = 0;
    int charBelowFvs = 0;
    ...
    for (int i = _length - 1; i >= 0; i--) {
      ...
      int charAbove;
      int currentChar = _inputWord.codeUnitAt(i);
  ...
  void _handleI(StringBuffer renderedWord,
      int positionInWord,
      int charAbove,
      int charBelow) {
    switch (_location) {
  ...
  bool _contextCallsForDoubleToothI(int positionInWord, int charAbove, int charBelow) {
    ...
    return ((charAbove == Unicode.OE ||
        charAbove == Unicode.UE) &&
  ...
  void _handleNA(StringBuffer renderedWord, int positionInWord,
      int charBelow, int charBelowFvs) {
        ...
        if (_fvs == Unicode.FVS1) {
          if (MongolCode.isFeminineVowel(charBelow) || charBelow == Unicode.I) {
            ...
            if (_gender == Gender.FEMININE) {
              renderedWord.insertCharCodeAtStart(Menksoft.MEDI_QA_FEM_CONSONANT_DOTTED);   // dotted feminine final before consonant
            } else {
              renderedWord.insertCharCodeAtStart(Menksoft.MEDI_QA_FVS1);     // dotted double tooth masculine
            }
            _glyphShapeBelow = Shape.TOOTH;
          ...
          if (MongolCode.isFeminineVowel(charBelow) || charBelow == Unicode.I) {
            ...
            if (_gender == Gender.FEMININE ||
              ...
              if (charBelow == Unicode.NA ||
                ...
                int renderedCharBelow = renderedWord.codeUnitAt(0);
  ...
  void _handleMA(StringBuffer renderedWord, int positionInWord,
      ...
      case Location.MEDIAL:
        if (_isRoundLetter(charAbove) ||
          ...
          if (_gender != Gender.MASCULINE ||
            ...
            charBelow == Unicode.MA || charBelow == Unicode.LA ||
  ...
  void _handleLA(StringBuffer renderedWord, int positionInWord,
      int charAbove, int charBelow) {
  ...
  void _handleYA(StringBuffer renderedWord, int positionInWord,
      int charAbove, int charBelow) {
  ...
  void _handleLHA(StringBuffer renderedWord, int positionInWord, int charAbove) {
    switch (_location) {
      ...
      case Location.MEDIAL:
        ...
        } else if (charAbove == Unicode.QA ||
            charAbove == Unicode.GA) {
          if (_gender == Gender.NEUTER) {
            _gender = MongolWord._getWordGenderAboveIndex(positionInWord, _inputWord);
          }
          if (_gender == Gender.FEMININE) {
  ...
  bool _isRoundLetterIncludingQG(int character) {
    return (character == Unicode.BA || character == Unicode.PA || character == Unicode.QA ||
        character == Unicode.GA || character == Unicode.FA || character == Unicode.KA ||
        character == Unicode.KHA);
  }

  bool _isRoundLetter(int character) {
    return (character == Unicode.BA || character == Unicode.PA || character == Unicode.FA ||
        character == Unicode.KA || character == Unicode.KHA);
  }
  ...
  static Gender getGender(String word) {
    ...
    int lastChar = word.codeUnitAt(length - 1);
